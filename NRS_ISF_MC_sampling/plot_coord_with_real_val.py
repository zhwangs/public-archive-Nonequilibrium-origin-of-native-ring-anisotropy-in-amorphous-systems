...
def ring_stat_hist(current_ring ):
    # make it log and take the ratio with respet to the initial diff.

    resulted_ring= current_ring 
    print('log')
    # init_bin_min=-5#bin_center-range_sigma#*np.std(np.log(round_arry))
    # init_bin_max=3#bin_center+range_sigma#*np.std(np.log(round_arry))
    # init_bin_size=120
...
def init_structure(replace=False):
    ...
    global max_step, max_l, min_l,total_sample_,bin_min_rough,bin_max_rough
    ...
    global full_coord_arry, angle_list_sample, round_arry,rough_arry
    ...
    round_arry=rough_round_arry[:,1]-1 # add -1 so that it is zero when no anisotropy
    rough_arry=rough_round_arry[:,2]
...
def update(frame):
    ...
    bin_min=-12#bin_center-range_sigma#*np.std(np.log(round_arry))
    bin_max=3#bin_center+range_sigma#*np.std(np.log(round_arry))
    ...
    data_x=bin_edges_true
    ax4.scatter(data_x,data_y/len(round_arry),s=20,c='tab:blue',alpha=0.3)
    ...
    ax4.plot(x_smooth,y_smooth,c='k')

    # max_y=np.max(y_smooth)
    # x_center_point=np.max(x_smooth[max_y==y_smooth])
    # half_y=max_y/np.sqrt(2)
    # y_left=y_smooth[x_smooth<x_center_point]
    # x_left=x_smooth[x_smooth<x_center_point]
    # y_right=y_smooth[x_smooth>x_center_point]
    # x_right=x_smooth[x_smooth>x_center_point]

    # y_check_left=np.min(np.abs(y_left-half_y))
    # y_check_right=np.min(np.abs(y_right-half_y))
    # if (y_check_left+y_check_right)<0.1:

    #     x_left_point=x_left[np.argmin(np.abs(y_left-half_y))]
    #     x_right_point=x_right[np.argmin(np.abs(y_right-half_y))]
    #     print('current_x_points')
    #     print(x_left_point)
    #     print(x_right_point)
    # else:
    #     print(y_check_left)
    ...
    #     x_left_point=x_left[np.argmin(np.abs(y_left-half_y))]
    #     x_right_point=x_right[np.argmin(np.abs(y_right-half_y))]
    #     print('Check left and right y-values for max_y/sqrt(2). The x_left and x_right are depending on them! ')
    # percentile_fit_arry.extend([x_left_point,x_center_point,x_right_point])
    # print('NSFKNSKJFNKSJ')
    # print(percentile_fit_arry)
    ...
    data_x=bin_edges_true
    ax4.scatter(data_x,data_y/len(round_arry_r),s=20,c='tab:red',alpha=0.3)
    ...
    data_y,_=hist_info(2*np.log(rough_arry),bins=bin_edges)
    data_x=bin_edges_true
    ax6.scatter(data_x,data_y/len(rough_arry),s=20,alpha=0.3,c='tab:blue')
    ...
    y_smooth=skewnorm_pdf(x_smooth,fitted_params_skewnorm[-4],fitted_params_skewnorm[-3],fitted_params_skewnorm[-2],fitted_params_skewnorm[-1])
    ax6.plot(x_smooth,y_smooth,c='k')
    # max_y=np.max(y_smooth)
    # x_center_point=np.max(x_smooth[max_y==y_smooth])
    # half_y=max_y/np.sqrt(2)
    # y_left=y_smooth[x_smooth<x_center_point]
    ...
    data_y,_=hist_info(2*np.log(rough_arry_r),bins=bin_edges)
    data_x=bin_edges_true
    ax6.scatter(data_x,data_y/len(rough_arry_r),s=20,alpha=0.3,c='tab:red')
   ...
   # ax6.set_title('sigma: '+str(np.round(fitted_params_skewnorm[-4],2))+', a: '+str(np.round(fitted_params_skewnorm[-3],2))+', a/sigma: '+str(np.round(fitted_params_skewnorm[-3]/fitted_params_skewnorm[-4],3)) )
    ...
    coord_file_path=parent_dir+'round_rough.dat'
    rough_round_arry=np.loadtxt(coord_file_path)
    init_diff_arry=2*np.log(rough_round_arry[:,3])
    ...
    data_x,data_y=ring_stat_hist(init_diff_arry)
    ...
    ax5.scatter(data_x,data_y/np.sum(data_y),c='tab:blue',s=20,alpha=0.3)
    
    x_smooth=np.linspace(init_bin_min,init_bin_max,1000)
    y_smooth=skewnorm_pdf(x_smooth,fitted_params_skewnorm[-4],fitted_params_skewnorm[-3],fitted_params_skewnorm[-2],fitted_params_skewnorm[-1])
    ax5.plot(x_smooth,y_smooth,c='k')
    # max_y=np.max(y_smooth)
    # x_center_point=np.max(x_smooth[max_y==y_smooth])
    # half_y=max_y/np.sqrt(2)
    # y_left=y_smooth[x_smooth<x_center_point]
    ...
    ax5.scatter(data_x,data_y/np.sum(data_y),c='tab:red',s=20,alpha=0.3)
    ...
    fig_file_path=parent_dir+'distance_distribution_.svg'
